...
def load_camera_calibration(calibration_dir: str):
    """
    Searches in dir_name for files names *.intrinsic.txt, *.distortion.txt, 
       and *.handeye.txt, loads then into corresponding arrays and returns them

    :param calibration_dir: Directory to search
       
    :returns intrinsics, distortion, handeye

    :raises FileNotFoundError: if the number of intrinsic or distortion 
        files is not 1
    """
    ...
    if os.path.isdir(calibration_dir):
        ...
        handeye_files = glob.glob(os.path.join(calibration_dir,
                                                  "*.handeye.txt"))
        if len(handeye_files) == 1:
            handeye_path = handeye_files[0]
    ...
    intrinsics = loadtxt(intrinsics_path)
    distortion = loadtxt(distortion_path)
    handeye = eye(4)
    if handeye_path is not None:
        handeye = loadtxt(handeye_path)

    return intrinsics, distortion, handeye
